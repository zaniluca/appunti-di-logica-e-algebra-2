...
\begin{proof}
    ...
    Vediamo da solo il caso in cui $F = \Diamond G$.
...
\begin{example}\
    \begin{itemize}
        \item L'ordinamento $\leq$ dei numeri naturali è una relazione antisimmetrica su $\mathbb{N}$.
        \item La relazione $x \mid y$ su $\mathbb{N}$ è antisimmetrica.
        \item La relazione $A \subseteq B$ su $\mathcal{P}(X)$ di un insieme $X$ è antisimmetrica.
    \end{itemize}
\end{example}
\begin{theorem}
...
\begin{proof}
    ...
    Da cui seguirebbe che $R_2$ è antisimmetrica, il che è falso. \Lightning
\end{proof}
...
\begin{equation*}
    \text{def}_\Diamond : \Diamond A \iff \neg \Box \neg A
\end{equation*}
è valido, $\vDash \text{def}_\Diamond$.
...
\begin{proof}
    ...
    Abbiamo quindi dimostrato che $\vDash \text{def}_\Diamond$
\end{proof}
...
\begin{definition}[Logica Modale Normale]
    ...
    \begin{enumerate}
        ...
        \item $\Gamma$ contiene tutte le istanze dello schema $\text{def}_\Diamond : \Diamond A \iff \neg \Box \neg A$
...
\begin{example}\
    \begin{itemize}
        \item Se $(S,R)$ è un frame, $\{F: (S,R) \vDash F\}$ è una logica normale.
        \item $\{F: \; \vDash F\}$ è una logica normale.
        \item Se $M$ è un modello su un frame $(S,R)$, $\{F: \; M \vDash F\}$ \underline{non} è una logica normale.
    \end{itemize}
\end{example}
    ...
    \begin{enumerate}
        \item Schemi di assiomi:
              \begin{enumerate}
                  ...
                  \item $\text{def}_\Diamond : \Diamond A \iff \neg \Box \neg A$
              \end{enumerate}
