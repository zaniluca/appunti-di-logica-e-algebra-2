...
\begin{document}
\subsection{Gruppo quoziente per anelli commutativi}
...
Con l'operazione
\[
    [x] \cdot [y] := [xy] \qquad \forall [x] , [y] \in \nicefrac{A}{I}
\]
abbiamo che $\nicefrac{A}{I}$ è un anello commutativo con unità $[1_A]$.

Infatti, mostriamo che l'operazione è ben definita: siano $x' \in [x]$ e $y' \in [y]$. Allora esistono $i_x \in I$ e $i_y \in I$ tali che
\begin{align*}
    x' & = x + i_x \\
    y' & = y + i_y
\end{align*}
...
\begin{align*}
    x'y' & = (x + i_x) (y + i_y)                                                                      \\
         & = xy + \underbrace{x i_y + y i_x + i_x i_y}_{\in I \text{ perchè $I$ è un ideale di $A$ }}
\end{align*}
Quindi $[x'y'] = [xy]$.

Inoltre $[1_A] [x] = [1_A x] = [x] $, per ogni $[x] \in \nicefrac{A}{I}$, quindi $[1_A]$ è l'unità di $\nicefrac{A}{I}$.
\begin{note}
    L'invertibilità si considera con rispetto per l'operazione di \textbf{moltiplicazione}, quindi $[x] \in \nicefrac{A}{I}$ è invertibile se esiste $[y] \in \nicefrac{A}{I}$ tale che $[x] \cdot [y] = [1_A]$.
\end{note}
\begin{example}
    ...
    \begin{itemize}
        ...
        \item $\mathbb{Z}_3 \simeq \{\overline{0}, \overline{1}, \overline{2}\}$ è un campo perchè $\overline{1}$ è invertibile e $\overline{2} \cdot \overline{2} = \overline{1}$, quindi anche $\overline{2}$ è invertibile.
              \begin{center}
                  ...
                  \end{tabular}
              \end{center}
        \item $\mathbb{Z}_4 = \{\overline{0},\overline{1},\overline{2},\overline{3}\}$ dove $\overline{2} \cdot \overline{2} = \overline{0} \neq \overline{1}$, quindi $\mathbb{Z}_4$ non è un dominio di integrità. In particolare non è un campo.
              \begin{center}
                  \renewcommand{\arraystretch}{1.25}
                  \begin{tabular}{ @{\hspace{8pt}}c@{\hspace{8pt}} | @{\hspace{8pt}}c@{\hspace{8pt}} | @{\hspace{8pt}}c@{\hspace{8pt}} | @{\hspace{8pt}}c@{\hspace{8pt}} | @{\hspace{8pt}}c@{\hspace{8pt}} }
...
\begin{example}
    ...
    Vediamo ora quali sono gli ideali di $\mathbb{Z}_n$.

    Un ideale di $\mathbb{Z}_n $ è un sottogruppo di $\mathbb{Z}_n$ e poiché $\mathbb{Z}_n$ è ciclico, i suoi sottogruppi sono ciclici e sono $\{\langle \overline{m} \rangle : \overline{m} \in \mathbb{Z}_n\}$.

    Verifichiamo quindi che $\langle \overline{m} \rangle \subseteq \mathbb{Z}_n$ è un ideale di $\mathbb{Z}_n$, $\forall \overline{m} \in \mathbb{Z}_n$.

    \begin{enumerate}[label=(\roman*)]
        \item $(\langle \overline{m} \rangle, +)$ è un sottogruppo di $\mathbb{Z}_n$.
        \item Se $\overline{a} \in \mathbb{Z}_n$ e $\overline{m} \in \langle \overline{m} \rangle$ allora
              \[
                  \overline{a} \cdot \overline{m} = \overline{am} = \underbrace{\overline{m} + \overline{m} + \ldots + \overline{m}}_{a \text{ volte }} \in \langle \overline{m} \rangle
              \]
    \end{enumerate}
    ...
    Inoltre, se $n > 1$
    \[
        \{\langle \overline{m} \rangle : \overline{m} \in \mathbb{Z}_n\} = \{\{\overline{0}\}, \mathbb{Z}_n\} \cup \{\langle \overline{m} \rangle : \MCD_{m \neq 0} \{m,n\} \neq 1\}
    \]

    Quindi $\mathbb{Z}_n$ è un campo se e solo se $\{\langle \overline{m} \rangle : \overline{m} \in \mathbb{Z}_n\} = \{\{\overline{0}\}, \mathbb{Z}_n\}$ (vedi \ref{prop:1.59}) se e solo se $n$ è un numero primo.
\end{example}
\begin{example}
...
\begin{definition}[Identità di Bézout]
    ...
    \]
    si chiama \textbf{identità di Bézout}.
\end{definition}
...
\begin{theorem}
    Siano $a,b \in \mathbb{N} \setminus \{0\}$, se $a \mid b$ ($a$ divide $b$), allora $a = \MCD\{a,b\}$. Se $a \nmid b$ e $r$ è l'ultimo resto non nullo dell'algoritmo di Euclide, allora $r = \MCD \{a,b\}$.

    Inoltre esistono $x,y \in \mathbb{Z} $ tali che
    \[
        ax + by = \MCD \{a,b\}
    \]
\end{theorem}
...
\]
\begin{proposition}
    Siano $a,b,c \in \mathbb{Z} $. Allora esistono $x,y \in \mathbb{Z} $ tali che $ax + by = c$ se e solo se $\MCD \{a,b\} \mid c$ ($\MCD \{a,b\}$ divide $c$).
\end{proposition}

\begin{proof}\
    \begin{itemize}
        \item ($\implies$) Se $ax + by = c$, allora $\MCD \{a,b\} \mid c$.
        \item ($\impliedby$) Viceversa, se $d := \MCD \{a,b\}$, allora abbiamo un'identità di Bézout
              \[
                  ax + by = d \qquad \forall x,y \in \mathbb{Z}
              \]
              Se $d \mid c$ cioè se $c = d \cdot k$ per qualche $k \in \mathbb{Z} $
              \[
                  a(kx) + b(ky) = kd = c
              \]
    \end{itemize}
...
\begin{example}
    L'equazione diofantea: $365x - 1876y = 24$ ha soluzione perchè $\MCD \{365,1876\} = 1$ e $1$ divide $24$.

    Avevamo l'identità di Bézout
    \[
        365(-699) - 1876(-136) = 1
    \]
    moltiplicando per 24 otteniamo $365(-699 \cdot 24) - 1876(-136 \cdot 24) = 24$.
...
\begin{example}
    ...
    Abbiamo che $\overline{365} \cdot \overline{a} = \overline{1}$ in $\mathbb{Z}_{1876}$ se e solo se esistono $a,b \in \mathbb{Z}$ t.c.
    \[
        365 \cdot a = 1 + b \cdot 1876 \iff 365 \cdot a - 1876 \cdot b = 1
    \]
...
\begin{remark}
    Se $p \in \mathbb{N}$ è un numero \textbf{primo}, scriviamo
    \[
        \boxed{
