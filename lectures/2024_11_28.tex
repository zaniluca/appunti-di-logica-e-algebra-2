...
\begin{example}
    $(\neg x_1 \lor x_2) \land x_1 \land (x_2 \lor x_3)$ è una formula CNF. Chiamiamola $F$. Allora:
    \begin{itemize}
        \item $\Var (F)=\{x_1,x_2,x_3\}$.
        \item Le clausole di $F$ sono $\{\neg x_1 \lor x_2, x_1, x_2 \lor x_3\}$.
        \item I letterali di $F$ sono $\{x_1, x_2, x_3, \neg x_1 \}$
    \end{itemize}
\end{example}
...
\begin{example}\
    \begin{itemize}
        \item La formula dell' esempio precedente è soddisfacibile
...
\begin{example}\
    \begin{itemize}
        \item La formula $x \lor \neg x$ è una tautologia.
        \item la formula dell' esempio precedente non è una tautologia.
    \end{itemize}
\end{example}

\begin{definition}[Conseguenza logica]
    Date due formule $F,G$ diciamo che $G$ è \textbf{conseguenza logica} di $F$, se per ogni assegnazione $V$ appropriata ad entrambe si ha che
    \[
        \boxed{
            V \vDash F \implies V \vDash G
        }
    \]
\end{definition}
\begin{example}
    La formula $G = y$ è conseguenza logica della formula $F = (\neg x \lor y) \land x$.

    Infatti l'unica assegnazione che soffisfa $F$ è $x \mapsto 1$, $y \mapsto 1$.

    Tale assegnazione soddisfa anche $G = y$.
\end{example}
...
\begin{definition}[Equivalenza logica]
    Due formule $F,G$ sono \textbf{logicamente equivalenti} se $F$ è conseguenza logica di $G$ e $G$ è conseguenza logica di $F$. In tal caso scriviamo $F \equiv G$.
\end{definition}
\begin{example}
    Nell'esempio precedente abbiamo visto che $G: = y$ è conseguenza logica di $F:= (\neg x \lor y) \land x $, che possiamo ricrivere come $(x \implies y) \land x$.

    Poiché $x \mapsto 0, y \mapsto 1$ soddisfa $y$ ma non $F$, abbiamo che $F$ non è conseguenza logica di $G = y$.
\end{example}
\begin{example}\
    \begin{itemize}
...
\begin{definition}[Frame]
    Un \textbf{Frame} è una coppia $(S,R)$, dove:
    \begin{itemize}
        \item $S$ è un insieme \underline{non vuoto} detto \textbf{insieme dei mondi}.
        \item $R \subseteq S \times S$ è una relazione su $S$, detta \textbf{relazione di accessibilità} (se $(x,y) \in R$ si dice che $y$ è accessibile da $x$).
    \end{itemize}
\end{definition}
