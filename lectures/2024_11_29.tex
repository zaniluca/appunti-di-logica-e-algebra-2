...
\begin{definition}[Modello]
    Un \textbf{modello} su un frame $(S,R)$ è una terna $(S,R,V)$ dove
    \[
        V : \Var \rightarrow \mathcal{P}(S)
    \]
    ci dice in quali mondi le variabili valgono 1. $V$ è detta \textbf{funzione di valutazione}.
\end{definition}
\begin{definition}
...
\begin{theorem}
    ...
    \[
        |S| \leq 2^{|F|} \qquad |F| = \text{lunghezza di $F$}
    \]
    Quindi il problema di soddisfacibilità di una formula modale è \textbf{decidibile}.
\end{theorem}

Se come frame prendiamo $(\{0\},R)$ dove $R \subseteq S \times S = \{(0,0)\}$ è quindi $S$ stesso, una funzione di valutazione è
\[
    V : \Var \rightarrow \mathcal{P}(S) = \{\emptyset, \{0\}\} \simeq \{0,1\}
\]
...
\begin{equation*}
    \Box x \implies x \qquad \text{"è necessario che $x$, allora $x$"}
\end{equation*}
...
Perchè sia sempre vera la formula $\Box x \implies x$, la relazione $R$ del frame deve essere \textbf{riflessiva}, ossia $(y,y) \in R$ $\forall y \in S$ (Vedi dopo per dimostrazione).
...
\begin{definition}
    Una formula $F$ si dice \textbf{vera in un modello M}, scritto \textbf{$M \vDash F$}, se
    \[
        M \vDash_x F \qquad \forall x \in S
    \]
...
\end{definition}
\begin{example}
    Con lo schema $\Box x \implies x$ si intendono tutte le formule di questa forma, come $\Box x \implies x$, $x$ variabile, o $\Box (\neg \Diamond \neg x) \implies \neg \Diamond \neg x$: abbiamo semplicemente sostituito $x$ con una formula qualsiasi (in questo caso $\neg \Diamond \neg x$).
\end{example}
...
\end{remark}
\begin{example}
    Vediamo ora che lo schema di formule
    \[
...
\begin{enumerate}
    \item
          \[
              \Box (A \land B) \iff (\Box A \land \Box B)
          \]
          $M \vDash_w \Box (A \land B)$ se e solo se $M \vDash_v A \land B, \forall v \in S$ t.c. $(w,v) \in R$,\\
          se e solo se $M \vDash_v A$ e $M \vDash_v B$ $\forall v \in S$ t.c. $(w,v) \in R$,\\
    ...
    \item
          ...
          \]
          $M \vDash_w \Diamond (A \lor B)$ se e solo se $M \vDash_v A \lor B, \exists v \in S$ t.c. $(w,v) \in R$,\\
    ...
    \item
          \[
              \Box (A \implies B) \implies (\Diamond A \implies \Diamond B)
          \]
          $M \vDash_w \Box (A \implies B)$ e $M \vDash_w \Diamond A$ implicano che $M \vDash_v (A \implies B), \forall v \in S$ t.c. $(w,v) \in R$,\\
    ...
    \item
          \[
              \Diamond (A \implies B) \implies (\Box A \implies \Diamond B)
          \]
          $M \vDash_w \Diamond (A \implies B)$ e $M \vDash_w \Box A$ implicano che $M \vDash_v (A \implies B), \exists v \in S$ t.c. $(w,v) \in R$, e $M \vDash_v A, \forall v \in S$ t.c. $(w,v) \in R$.
...
\newpage
Mostriamo che i seguenti schemi di formule \underline{non sono validi}, ossia che esistono frame su cui non sono validi. La relazione $R$ del frame è rappresentata tramite il grafo.

Ovviamente ci potrebbero essre altri frame oltre a quelli proposti come controesempi.
\begin{enumerate}
    \item $\Diamond A \implies \Box A$
          ...
          Scelgo $V(x) = \{2\} $ (ossia la formula $x$ è vera solo nel mondo 2) quindi:
          \begin{itemize}
              \item $M \vDash_1 \Diamond x$: esiste un mondo accessibile da 1 in cui $x$ è vera (il mondo 2)
              \item $M \nvDash_1 \Box x$: non tutti i mondi accessibili da 1 soddisfano $x$ (se da 1 si va in 1, $x$ non è vera)
          \end{itemize}
          quindi $M \nvDash_1 (\Diamond A \implies \Box A)$
    ...
    \item $\Box A \implies \Box \Box A$
          ...
          Scelgo $V(x) = \{2\}$ Quindi $M \vDash_1 \Box x$.
    ...
    \item $\Box (A \implies B) \implies (\Box A \implies \Diamond B)$
          ...
          Scelgo $V(A) = V(B) = \{1\}$ quindi
          \begin{itemize}
              \item $M \vDash_2 \Box (A \implies B)$
              \item $M \vDash_2 \Box A$
          \end{itemize}
          ma $M \nvDash_2 \Diamond B$, perchè non esiste $v \in \{1,2\}$ t.c $M \vDash_v B$ e $(2,v) \in R$
    \item $\Box (\Box A \implies B) \lor \Box (\Box B \implies A)$.

          Prendiamo $A = B = x$ nel frame
          \begin{equation*}
              \begin{tikzpicture}[->]
                  \node (1) {1};
                  \node (2) [right of=1] {2};
    ...
    \item $\Box (A \lor B) \implies (\Box A \lor \Box B)$
          ...
          $V(x) = \{1\}, V(y) = \{2\}$ $M \vDash_1 \Box (x \lor y)$ se e solo se $M \vDash_1 (x \lor y)$ e $M \vDash_2 (x \lor y)$.

          Quindi abbiamo che $M \vDash_1 \Box (x \lor y)$.
...
\begin{proof}\
    \begin{itemize}
        \item ($\implies$) Se $R$ non fosse riflessiva ci sarebbe un mondo $y \in S$ tale che $(y,y) \notin R$.

              Sia $Z = V(x)$, $Z \subseteq S,$ insieme dei mondi in cui la $x$ è vera, prendiamo $y$ tale che $y \notin Z$ (cioè $x$ è falsa nel mondo $y$).

              Sia $Z \supseteq \{z \in S : (y,z) \in R\}$ cioè $x$ sia vera in tutti i mondi accessibili da $y$.

              Allora se $M = (S, R, V)$, $M \vDash_y \Box x$ ma $M \nvDash_y x$ \Lightning.

        \item ($\impliedby$) Sia $R$ riflessiva, sia $M \vDash_w \Box A$, quindi $M \vDash_v A$ $\forall v \in S$ t.c. $(w,v) \in R$, quindi $M \vDash_w A$ perchè per la riflessività $(w,w) \in R$.
    \end{itemize}
\end{proof}
\begin{theorem}
...
\begin{proof}\
    \begin{itemize}
        \item ($\implies$) Assumiamo che lo schema $A \implies \Box \Diamond A $ sia valido in $(S,R)$.
              ...
              Quindi esiste $r \in S$ t.c. $(t,r) \in R$ e $M \vDash_r x$, ossia $r = s$, quindi $R$ è simmetrica.
        \item ($\impliedby$) Sia $R$ simmetrica, ossia $(x,y) \in R \implies (y,x) \in R$.

              Sia $M \vDash_w A$ e $(w,v) \in R$. Per simmetria $(v,w) \in R$, quindi $M \vDash_v \Diamond A$ $\forall v \in S$ t.c. $(w,v) \in R$, ossia $M \vDash_w \Box \Diamond A$.
    \end{itemize}
\end{proof}
\begin{theorem}
...
\begin{proof}\
    \begin{itemize}
        \item ($\implies$) Assumiamo che sia valido lo schema $\Box A \implies \Box \Box A$ su un frame $(S,R)$.

              Sia $x$ una variabile, $s \in S$ e $V(x) = \{w \in S : (s,w) \in R\}$. ($x$ è vera in tutti i mondi accessibili da $s$).
              ...
              Da ciò segue che $r \in V(x)$ ossia $(s,t) \in R$ e $(t,r) \in R \implies (s,r) \in R$, quindi $R$ è transitiva.
        \item ($\impliedby$) Sia $R$ transitiva, ossia $(x,y) \in R, (y,z) \in R \implies (x,z) \in R$.
