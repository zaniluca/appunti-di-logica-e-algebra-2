...
\esercizio{%
    ...
    \begin{enumerate}
        ...
        \item Per la validità in $(S,R)$, dobbiamo verificare la formula per ogni valutazione possibile, chiaramente da ogni punto possiamo raggiungere un punto dove $X \lor Y$ è vero (almeno una variable è vera). Quindi $(S,R) \models \Box\Box\Box(X \lor Y)$ è \textbf{vero}.

        \item Similmente, da qualsiasi punto, possiamo, in 3 step raggiungere un punto dove $\neg X \land Y$ è vero, infatti in 3 mosse possiamo sempre raggiungere $d$. Quindi $(S,R) \models \Diamond\Diamond\Diamond(\neg X \land Y)$ è \textbf{vero}.
    \end{enumerate}
}
